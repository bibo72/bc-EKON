...
$('.flyout-pagination-container .next').click(function(){   
    setTimeout(function() {
        if($('.navPage-subMenu-list div:last-child').hasClass('active-page')) {
            $('a.navPages-action.next').addClass('inactive');
        }
    }, 150);
});
$(document).ready(function() {
    $('.navPages-item').hover(function() {
        var items = $(this).find('.desktop .navPage-subMenu div.desktop .navPage-subMenu-list div.flyout.page').length;
        console.log(items);
        if (items <= 1) {
            $(this).find('.desktop .navPage-subMenu a.next').addClass('inactive');
        }
    });
});
